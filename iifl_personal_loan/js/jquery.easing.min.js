...
//	var regex = new RegExp("^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]*$");
...
//	return false;
//});
if(!$('#error-user').hasClass("api-err")) {
  $('#error-user').hide();
}
...
function chkmyfield(){
	...
	$("#textfield").val($("#textfield").val().replace(/  +/g, ' '));
	
	var compname_length = $("#textfield").val().length;	
	if(compname_length <4  || compname_length >50 || $("#textfield").val().indexOf("  ")!=-1)	
	{
	   $('#error-user').html('Company name must be more than 3 characters long');
	   $('#error-user').fadeIn();
	   erroruser1 = 0;
	}
	else {
...
$(document).on("click", ".next", function(){
	...
	current_fs = $(this).parent().parent().parent().parent();
	next_fs = $(this).parent().parent().parent().parent().next();
	console.log(typeof $('#salary').val().replace(/,/g, ""))
	if(($('.screen02bg:visible').length > 0) && ($('#salary').val().replace(/,/g, "") < 35000)){
		console.log("if")
		$(".eerr").show();
...
(function($) {
  ...
  $(document).on('focusin', 'input, textarea', function() {
      $(this).prev('span').css({'color': '#6f9db9'});
  });
  $(document).on('focusout', 'input, textarea', function() {
      $(this).prev('span').css({'color':'#215b98'});
  });
...
$(document).ready(function () {
	...
	$('.updadepannel').click(function(){
		$('.updateloadn-detailbox').slideToggle();	
	});
	
    $("#updatehide").mouseleave(function(){
  $(".updateloadn-detailbox").slideUp();
    });
});
