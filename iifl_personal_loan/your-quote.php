...
<?php
	...
	if(isset($_SESSION['co_applicant_details'])) {
		$_SESSION['CIBIL']['revised_ROI'] = $_SESSION['co_applicant_details']['CIBIL']['ROIActual'];
	}
	...
	$co_app_eligible = true;
	
	if(isset($_SESSION['co_applicant_details'])) {
		if(isset($_SESSION['co_applicant_details']['CIBIL']['flag']) && $_SESSION['co_applicant_details']['CIBIL']['flag'] == 'Yes') {
			$loan_amount					= xss_filter($_SESSION['personal_details']['appliedloanamt']);
			$tenure								= xss_filter($_SESSION['personal_details']['tenure']);
			$revised_max_amount		= xss_filter($_SESSION['co_applicant_details']['CIBIL']['MaxAmount']);
			$revised_tenure				= xss_filter($_SESSION['co_applicant_details']['CIBIL']['MaxTenure']);
			$cibil_tenure					= $revised_tenure/12;
			$revised_emi					= xss_filter($_SESSION['co_applicant_details']['CIBIL']['CIBILTotalEMI']);
			$revised_pro_fee			= xss_filter($_SESSION['co_applicant_details']['CIBIL']['ProcessingFeeActual']);
			$revised_roi					= xss_filter($_SESSION['co_applicant_details']['CIBIL']['ROIActual']);
			$loan_interest 				= xss_filter($_SESSION['co_applicant_details']['CIBIL']['ROIActual']);	
		} else {
			$co_app_eligible = false;
		}
	}
	
	//echo $revised_max_amount.' - '.$revised_tenure.' - '. $revised_roi;
	//echo "<pre>"; print_r($_SESSION); exit;
	$interest = $revised_roi / 1200;
	// echo "<pre>-"; print_r($interest); echo "<pre>--"; print_r($revised_roi); echo "<pre>---"; print_r($tenure); exit;
	//$emi = $_SESSION['personal_details']['actualloanEMI'] = ceil($interest * -$revised_max_amount * pow((1 + $interest), $tenure) / (1 - pow((1 + $interest), $tenure)));
	$emi = $_SESSION['personal_details']['actualloanEMI'] = calculate_emi($revised_max_amount, $revised_roi, $revised_tenure);
	$error = false;
	if(isset($_POST['cosubmit'])) {
		...
		if(isset($_SESSION['co_applicant_details'])) {
			$processing_fee 			= $_SESSION['co_applicant_details']['CIBIL']['processing_fee'] = ceil(($_SESSION['co_applicant_details']['CIBIL']['ProcessingFeeActual'] * $loan_amount) / 100);
		}
		...
		redirect_to('co-applicant-detail');
	}
	if(isset($_POST['submit'])) {
		...
		$_SESSION['personal_details']['tenure'] = $loan_tenure * 12;		
		if(!isset($_SESSION['co_applicant_details'])) {
			$roi = $_SESSION['CIBIL']['revised_ROI'];
		} else {
			$roi = $_SESSION['co_applicant_details']['CIBIL']['ROIActual'];
		}
		//echo "<pre>"; print_r($_SESSION); exit;
		if(!$error) {
...
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en" class="no-js"><!--<![endif]--> 
		...
		<script>
			$(function() {
				...
				$("#SliderPoint3" ).slider({
					...
					$( "#SliderPoint4" ).slider({
			      value:parseInt("<?php echo $revised_tenure/12; ?>"),
			      min: 1,
			      max: <?php echo $revised_tenure/12; ?>,
			      step: 1,
			      slide: function( event, ui ) {							
							$( "#years" ).val(ui.value+' Years');
							PointerFN(event.target.id,"years",0,<?php echo $revised_tenure/12; ?>,' Years');
							setTimeout(function(){
				...
				/*$("#SliderPoint4" ).slider({
					...
					slide: function( event, ui ) {
						...
						PointerFN(event.target.id,"years",0,<?php echo $cibil_tenure; ?>,' Years');
						setTimeout(function(){
							var loanAmt = $("#nLoanAmt").val();
							//$("#SliderPoint3").slider( "value",removeComa($("#loan").val()));
							$('#SliderPoint3').slider("option", "max", removeComa(loanAmt));
							var ln_amt = $("#loan").val();
	...
	<body class="bodyoverflow your_quote" onLoad="form1.reset();">
		<!-- Your Quote Pixel -->
		...
		<form id="form1" name="form1" method="post" action="your-quote">
				...
				<!--body home-->
					...
					<div id="msform">
						<section class="body-home-outer myclass screen05bg coapp-uquote-container">
							<div class="innerbody-home heightdesign2 ur-quote-height ipadhfull">
								<div class="eligibleDetail"><!-- eligibleDetail part start -->
									<div class="edNav">Congratulations<b class="orange">
										<?php echo $_SESSION['personal_details']['applicantname']; ?>!</b>
										<strong>You are eligible for a Loan of</strong> &nbsp;
										<span style="display:inline-block"><span class="rsapply">`</span> <?php echo to_rupee($revised_max_amount); ?></span>
										<a href="javascript:;" class="applyNow" id="applyBtn">APPLY NOW</a>
									</div>
									<?php
										if(isset($_SESSION['co_applicant_details']))	{
											if(isset($_SESSION['co_applicant_details']['CIBIL']['flag']) && $_SESSION['co_applicant_details']['CIBIL']['flag'] != 'Yes') {
									?>
										<div class="edNav">Sorry, Your Co - Applicant is not eligible.</div>
									<?php } }?>
									...
									<div class="edContant">
										<aside> <span class="edcLine1">Loan Amount</span>
											...
											<input type="text" name="loanAmount" class="edcLine2 inventloan inventloan_quote" value="<?php echo to_rupee(	$revised_max_amount); ?>" id="loan"  maxlength="8" onkeypress="return isNumberKey(event)" onChange="setPoint('loan','SliderPoint3',<?php echo $_SESSION['personal_details']['minloanamt']; ?>,<?php echo num_only($revised_max_amount); ?>,'');"> 
											<input type="hidden" id="nLoanAmt" value="<?php echo to_rupee($revised_max_amount); ?>" />
											
											<div class="edcRange no-sliderbg">
												<span class="edcRangeIcons inventslider" id="SliderPoint3"><!-- <img src="images/slider-icon.png" class="scaleX"> --></span>
											</div>
											<span class="edcRangeLeft"><b>`</b><?php echo $_SESSION['personal_details']['minloanamt']; ?></span>
											<span class="edcRangeRight"><b>`</b> <span class="myval1 rightamt"><?php echo to_rupee($revised_max_amount); ?></span></span>
			...
			<input type="text" name="loanTenure" class="edcLine2 tenureinventslide" value="<?php echo $revised_tenure/12; ?> Years" id="years"  maxlength="7" onkeypress="return isNumberKey(event)" onChange="setPoint('years','SliderPoint4',1,<?php echo $revised_tenure/12; ?>,' Years');">
			<input type="hidden" id="nTenure" value="<?php echo $revised_tenure/12; ?>">
			...
			<span class="edcRangeLeft">1 Years</span><span class="edcRangeRight"><?php echo $revised_tenure/12; ?> Years</span> <span class="edcAdjust">Adjust the loan tenure to an<br>
			EMI that suits you.. </span> </aside>
		<div class="clr1"></div>
		<aside class="design2emi"> <span class="edcLine1">EMI</span><br clear="all" />
		<div class="out-cover-emi posit_emitx">
		...
		<?php } else { ?>
		<h3 id="roi"><?php echo $_SESSION['co_applicant_details']['CIBIL']['ROIActual']; ?>%</h3>
		<?php }?>
			<input type="hidden" id="roiAct" value="<?php if(!isset($_SESSION['co_applicant_details'])) { echo $_SESSION['CIBIL']['revised_ROI']; } else { echo  $_SESSION['co_applicant_details']['CIBIL']['ROIActual'];}?>" name="roiAct">
		</div>
		<div class="edBottomBox">
			<h1>Processing fee</h1>
			<h3><?php echo $revised_pro_fee; ?>%</h3>
			<input type="hidden" name="proFeeActual" value="<?php echo $revised_pro_fee; ?>" id="proFeeActual">
		</div>
		...
		<div class="clr"></div>
			...
			<input type="hidden" name="webpageno" id="webpageno" value="<?php if(isset($_SESSION['co_applicant_details'])) echo '11'; else echo '6'; ?>">
				<input type="submit" name="submit" id="submitBtn"  value="GET INSTANT APPROVAL" class="homesubmit">
				
				<?php if(!isset($_SESSION['co_applicant_details']['CIBIL'])): ?>
					<div class="co-app-cta">
						<span>or</span>
						<p>Looking for a higher loan amount? Increase your eligibility by adding a co-applicant.</p>
						<!--<div class="add-co-app-btn"><input type="submit" name="cosubmit" id="cosubmitBtn"  value="ADD CO-APPLICANT" class="homesubmit"><!-- <a href="co-applicant-detail.php">ADD CO-APPLICANT</a> --><!--</div>-->
						<div style="margin-top:15px;"><input type="submit" name="cosubmit" id="cosubmitBtn"  value="ADD CO-APPLICANT" class="homesubmit"><!-- <a href="co-applicant-detail.php">ADD CO-APPLICANT</a> --></div>
					</div>
				<?php endif; ?>
