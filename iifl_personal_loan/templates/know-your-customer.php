...
<?php session_start(); ?>
<?php
	//if(isset($_POST['kycSubmit'])) { print_r($_POST); }
	$error = '';
	$pan_error = '';
	...
	if(isset($_SESSION['customer']['AddharNo'])) {
		...
		if(isset($_SESSION['customer']['City'])) {
			...
			$_SESSION['personal_details']['perm_city_code'] = xss_filter($_SESSION['customer']['City']);
			$_SESSION['personal_details']['perm_city_code'] = xss_filter($_SESSION['customer']['City']);
		} 
			
		if(isset($_SESSION['customer']['State'])) {
			$state = xss_filter($_SESSION['customer']['State']);
			$_SESSION['personal_details']['perm_state_code'] = xss_filter($_SESSION['customer']['State']);
			$_SESSION['personal_details']['perm_state_code'] = xss_filter($_SESSION['customer']['State']);
		} 
	...
	if(isset($_POST['kycSubmit'])) {
		...
		if($_POST['currentaddress1'] == '') {
			...
			if(!valid_address($_POST['currentaddress1'])) {
				...
				$c_addr1 = xss_filter($_POST['currentaddress1']);
			}
		}
		...
		if($_POST['currentaddress2'] != '') {
			if(!valid_address($_POST['currentaddress2'])) {
				...
				$c_addr2 = xss_filter($_POST['currentaddress2']);
			}
		}
		...
		if($_POST['currentstate'] == '') {
			...
			if(!valid_location($_POST['currentstate'])) {
				...
				$c_state = xss_filter($_POST['currentstate']);
			}
		}
		...
		if($_POST['currentcity'] == '') {
			...
			if(!valid_location($_POST['currentcity'])) {
				...
				$c_city = xss_filter($_POST['currentcity']);
			}
		}
		...
		if(!$error) {
			...
			if(isset($_SESSION['customer']['aadharNo'])) {
				...
				//echo '<pre>'; echo print_r($json2); echo '</pre>';
...
<head>
	...
	<style>
		.join-date #error-user{position:relative; left:196px;}
  ...
  $("#f").click();
			...
			$(".block2 .detailfields").each(function() {
				if($(this).find("#error-user").is(':visible')) {
					$(".block2").show();
				}
			});
...
<div id="main-wrap">
 <!--popup-->
  ...
  <div class="loan-amount">Loan Amount -
								<span class="orange"><div><b class="rupee-symb">`</b>
									<b id="loanAmu"><?php echo to_rupee($_SESSION['personal_details']['appliedloanamt']); ?></b></div>
								</span><br>
    Loan Tenure -
								<span class="orange" id="tenure">
									<?php echo $_SESSION['personal_details']['tenure']/12 ; ?> Years</span>
							</div>
  <div class="loan-amount loanamout-small">Rate of Interest -
    ...
    Processing Fees -
								<span class="orange"><b class="rupee-symb">`</b>
									<?php echo to_rupee($_SESSION['personal_details']['processing_fee']); ?></span>
							</div>
  <div class="total-emi">EMI -
								<span class="orange"><b class="rupee-symb">`</b>
									<b id="emidiv"><?php echo to_rupee($_SESSION['personal_details']['actualloanEMI']); ?></b>
								</span>
							</div>
  ...
  <div id="block1">
    ...
    <input type="text" name="applicantname" id="applicantname" value="<?php echo $first_name; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?>/>
											<?php if($err_fn != ''): ?><div id="error-user"><?php echo $err_fn; ?></div><?php endif; ?>
										</label>
      </div>
      <div class="detailfields">
        <label class="input"> <span>Middle Name</span>
    <input type="text" name="middlename" id="middlename" value="<?php echo $middle_name; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?>/>
											<?php if($err_mn != ''): ?><div id="error-user"><?php echo $err_mn; ?></div><?php endif; ?>
        </label>
      </div>
      <div class="detailfields">
        <label class="input"> <span>Last Name*</span>
    <input type="text" name="lastname" id="lastname" value="<?php echo $last_name; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?>/>
											<?php if($err_ln != ''): ?><div id="error-user"><?php echo $err_ln; ?></div><?php endif; ?>
										</label>
      </div>
      <div class="detailfields">
        <label class="input"> <span>PAN Card Number*</span>
    <input type="text" name="panno" id="panno" maxlength="10" value="<?php echo $panno; ?>"/>
      ...
      <div class="detailfields dob">
        <input name="dob" type="text" class="inputdob" value="<?php echo !empty($dob)?$dob:'Date of Birth*'; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'disabled':''; ?> onFocus="if (this.value=='Date of Birth*') this.value = ''" onBlur="if (this.value=='') this.value = 'Date of Birth*'" id="datepicker-example2">
      ...
      <div class="clr"></div>
	    ...
	    <div class="personalinfo-radiobox">
	      ...
	      <div class="chkpersonal-info">
													<?php if($gender == 'F'): ?>
														<input type="radio" name="radioGe" id="g" value="F" checked="checked" readonly maxlength="1" >
													<?php else: ?>
														<input type="radio" name="radioGe" id="g" value="F" maxlength="1" <?php echo isset($_SESSION['customer']['aadharNo'])?'disabled':''; ?>>
													<?php endif; ?>
  ...
  <div id="block3">
    ...
    <div class="detailwrap-line1 join-date">
      <div class="empdetail-box"> When did you join <?php echo $_SESSION['personal_details']['companyname']; ?>: <span class="empdetail-date">
        <input type="text" name="curWorkPlace" class="empdate inputdob" id="datepicker-example1"/> <span class="mandatory">*</span>
  ...
  <div id="block4">
    ...
    <input type="text" id="permanentaddress1" name="permanentaddress1" value="<?php echo $addr1; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?>/>
    ...
    <input type="text" id="permanentaddress2" name="permanentaddress2" value="<?php echo $addr2; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?> />
    ...
    <input type="text" id="permanentaddress3" name="permanentaddress3" value="<?php echo $addr3; ?>" <?php echo isset($_SESSION['customer']['aadharNo'])?'readonly':''; ?>/>
											<?php if($err_paddr3 != ''): ?><div id="error-user"><?php echo $err_paddr3; ?></div><?php endif; ?>
										</label>
      </div>
      <div class="detailfields">
        <label class="input"> <span>Pin Code*</span>
  ...
  <input type="submit" name="kycSubmit" id="submitBtn" value="NEXT"  class="homesubmit" />
      </div>
    </div>
