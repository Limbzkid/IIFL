<?php
  ...
  return 0;
	...
	function blacklisted_email($email) {
		$arr = array('yahoo.com', 'yahoo.co.in', 'gmail.com','hotmail.com', 'live.com');
		$temp = explode('@', $email);
		$domain = $temp[1];
	...
	/*
	if(!preg_match("/^([a-zA-Z0-9.\-']+\s?)*$/", $design) && $design != '') {
			$msg[] = "designErr-* Invalid entry in Designation field.";
			$err_status = 1;
		}
		
		if(!preg_match("/^([a-zA-Z.\']+\s?)*$/", $country) && $country != '') {
			$msg[] .= "countryErr-* Invalid entry in Country field.";
			$err_status = 1;
		}
